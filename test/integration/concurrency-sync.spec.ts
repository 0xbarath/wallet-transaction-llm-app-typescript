...
describe('Concurrency Sync (Integration)', () => {
  ...
  it('should reject sync when already in progress (409)', async () => {
    const walletId = await registerWallet();

    // Manually set syncInProgress flag
  ...
  it('should succeed sync after lock is released', async () => {
    const walletId = await registerWallet();

    // Set and then release the lock
    await prisma.walletSyncState.update({
      where: { walletId },
      data: { syncInProgress: true, version: { increment: 1 } },
    });
